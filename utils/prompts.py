...
3. **simple_aggregation**: 단순 집계 (SQL만으로 완결)
   - 예: "전국 평균 인구수는?", "인구가 가장 많은 도시는?", "20대와 60대 차이의 월평균은?"
   - 특징: SQL 집계 함수(SUM, AVG, MAX, MIN, COUNT)와 사칙연산으로 해결

4. **derived_calculation**: 파생 계산 (SQL 결과 받아서 추가 계산)
   - 예: "서울 2020년 대비 2023년 인구 증가율은?", "남녀 성비는?", "인구밀도는?"
   - 특징: SQL은 기본 값만 반환, 증가율/비율/밀도 등은 Python에서 계산 필요
...
위 정보를 바탕으로, 학술 논문 초록(또는 요약) 형식의 글을 작성하세요.
