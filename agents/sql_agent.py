...
- 합계가 필요하면: SUM() 함수 사용
- 평균이 필요하면: AVG() 함수 사용
- 개수가 필요하면: COUNT() 함수 사용
- 비율/백분율이 필요하면: SQL에서 직접 나눗셈 계산 (예: CAST(SUM(값1) AS FLOAT) / SUM(값2))
- 여러 값을 더하거나 빼야 한다면: SQL의 사칙연산 사용
- 절대로 SQL 결과를 받아서 직접 계산하지 마세요
- 모든 계산은 SQL 쿼리 안에서 완료되어야 합니다

예시:
- 나쁜 예: SELECT 값 FROM 테이블 → 결과를 받아서 직접 더하기
- 좋은 예: SELECT SUM(값) FROM 테이블
- 나쁜 예: 두 쿼리 결과를 받아서 나누기
- 좋은 예: SELECT CAST(SUM(값1) AS FLOAT) / SUM(값2) FROM 테이블
...
5. 데이터 출처 테이블을 언급하세요
6. SQL 쿼리에서 계산된 결과를 그대로 사용하고, 절대 직접 계산하지 마세요
