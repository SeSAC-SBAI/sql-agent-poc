...
def search_tables_from_db(
    ...
    Args:
        query: 사용자 질문
        n_results: 반환할 테이블 수
        threshold: 거리 임계값 (낮을수록 유사함, 기본값 1.5)

    Returns:
        list: 관련 테이블 정보 리스트 (거리가 threshold 이하인 것만)
    """
    embeddings = UpstageEmbeddings(
        api_key=settings.UPSTAGE_API_KEY, model="solar-embedding-1-large"
    )
    ...
    for doc, distance in results:
        # ChromaDB는 거리(distance)를 반환 - 낮을수록 유사함
        # threshold보다 작으면 유사한 것으로 판단
...
if __name__ == "__main__":
    ...
    for query in test_queries:
        results = search_tables_from_db(query, n_results=1)
