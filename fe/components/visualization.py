...
from fe.styles.premium import PREMIUM_COLORS
...
def create_chart(df: pd.DataFrame, viz_metadata: Dict[str, Any]) -> Optional[go.Figure]:
    ...
    """
    try:
        print(f"[DEBUG create_chart] df type: {type(df)}, empty: {df.empty if hasattr(df, 'empty') else 'N/A'}")
        ...
        print(f"[DEBUG create_chart] chart_type: {chart_type}, x_col: {x_col}, y_col: {y_col}")
        print(f"[DEBUG create_chart] df.columns: {list(df.columns)}")
        
        if x_col not in df.columns or y_col not in df.columns:
            print(f"[DEBUG create_chart] 컬럼이 DataFrame에 없음. x_col={x_col}, y_col={y_col}")
            return None
